 ...
 * @constructor
 */
function WaterCanvas(width, height, documentElement, waterModel, props) {
 ...
 * @constructor
 */
function WaterModel (width, height, props) {
	// If a certain property is not set, use a default value
	props = props || {};
...
WaterModel.prototype.getWater = function(x, y){
	var xTrans = x/this.resolution;
	var yTrans = y/this.resolution;

	if(!this.interpolate || this.resolution==1.0){
		var xF = Math.floor(xTrans); 
		var yF = Math.floor(yTrans);	
	...
	var xF = Math.floor(xTrans); 
	var yF = Math.floor(yTrans);
	var xC = Math.ceil(xTrans); 
	var yC = Math.ceil(yTrans);	
 ...
 */
function RainMaker(width, height, waterModel, raindrop2dArray) {
	this.width = width;
	this.height = height;
	this.waterModel = waterModel;
...
module.exports = { WaterCanvas, WaterModel, RainMaker };
